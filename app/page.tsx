  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <section style={{ padding: '80px 40px 60px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <h1 className="fade-up-2" style={{ fontSize: 'clamp(32px, 5vw, 64px)', fontWeight: 300, lineHeight: 1.1, letterSpacing: -1.5, marginBottom: 24, color: 'var(--text)' }}>
        ...
        {/* STATS */}
        <div className="fade-up-4" style={{ display: 'flex', gap: 1, background: 'var(--border)', borderRadius: 16, overflow: 'hidden', maxWidth: 560 }}>
          {[
            ...
            <div key={i} style={{ flex: 1, background: 'var(--bg3)', padding: '18px 20px' }}>
      ...
      <section style={{ padding: '32px 40px 0', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            ...
            style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 10, padding: '9px 16px', fontSize: 13, color: 'var(--text)', outline: 'none', width: 220, fontFamily: 'DM Sans, sans-serif' }}
          />
          <select value={fundType} onChange={e => setFundType(e.target.value)}
            style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 10, padding: '9px 14px', fontSize: 13, color: fundType ? 'var(--text)' : 'var(--text3)', outline: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
            <option value="">Tüm Türler</option>
            {fundTypes.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
          <select value={risk} onChange={e => setRisk(e.target.value)}
            style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 10, padding: '9px 14px', fontSize: 13, color: risk ? 'var(--text)' : 'var(--text3)', outline: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
            <option value="">Tüm Riskler</option>
            {[1,2,3,4,5,6,7].map(r => <option key={r} value={r}>Risk {r}/7</option>)}
          </select>
          <select value={sort} onChange={e => setSort(e.target.value)}
            style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 10, padding: '9px 14px', fontSize: 13, color: sort ? 'var(--text)' : 'var(--text3)', outline: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
          ...
          {(search || fundType || risk || sort) && (
            <button onClick={() => { setSearch(''); setFundType(''); setRisk(''); setSort('') }}
              style={{ background: 'transparent', border: '1px solid var(--border2)', borderRadius: 10, padding: '9px 14px', fontSize: 13, color: 'var(--text3)', cursor: 'pointer' }}>
              ✕ Temizle
            </button>
          )}
          <span style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--text3)', fontFamily: 'DM Mono, monospace' }}>{filtered.length} fon</span>
        </div>
      </section>

      {/* FUNDS */}
      <section style={{ padding: '24px 40px 60px', maxWidth: 1200, margin: '0 auto' }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--text3)', fontSize: 14 }}>Sonuç bulunamadı.</div>
        ) : (
              ...
              <a key={fund.code} href={`/fon/${fund.code?.toLowerCase()}`}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                  <div>
                    ...
                    {fund.fundType && <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 6 }}>{fund.fundType}</div>}
                  </div>
