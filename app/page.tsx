...
function AiRadar({ radarData }: { radarData: any }) {
  if (!radarData || (!radarData.positives?.length && !radarData.warnings?.length)) return null
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20, margin: "32px 0" }}>
      <div style={{ background: "rgba(0,240,128,0.03)", border: "1px solid rgba(0,240,128,0.15)", borderRadius: 20, padding: 24 }}>
        <div style={{ color: "var(--green)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", background: "var(--green)" }} /> AI RADARI: POZİTİF İVME
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {radarData.positives.map((f: any) => (
            <a key={f.code} href={`/fon/${f.code.toLowerCase()}`} style={{ textDecoration: "none", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(255,255,255,0.03)", padding: "12px 16px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)" }}>
              <span style={{ fontWeight: 700, color: "var(--text)", fontFamily: "DM Mono, monospace", fontSize: 14 }}>{f.code}</span>
              <span style={{ color: "var(--green)", fontWeight: 700, fontFamily: "DM Mono, monospace" }}>+{f.change.toFixed(2)}% 🚀</span>
            </a>
          ))}
        </div>
      </div>
      <div style={{ background: "rgba(255,68,68,0.03)", border: "1px solid rgba(255,68,68,0.15)", borderRadius: 20, padding: 24 }}>
        <div style={{ color: "var(--red)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", background: "var(--red)" }} /> AI RADARI: TREND ZAYIFLIĞI
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {radarData.warnings.map((f: any) => (
            <a key={f.code} href={`/fon/${f.code.toLowerCase()}`} style={{ textDecoration: "none", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(255,255,255,0.03)", padding: "12px 16px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)" }}>
              <span style={{ fontWeight: 700, color: "var(--text)", fontFamily: "DM Mono, monospace", fontSize: 14 }}>{f.code}</span>
              <span style={{ color: "var(--red)", fontWeight: 700, fontFamily: "DM Mono, monospace" }}>{f.change.toFixed(2)}% ⚠️</span>
            </a>
          ))}
        </div>
      </div>
...
export default function Home() {
  ...
  const top5: any = top5Data
  const top5_1m: any[] = top5.top5_1m || []
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <section className="hero-section" style={{ padding: '40px 5% 32px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div className="fade-up-4 stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 24 }}>
...
<section style={{ padding: '0 5%', maxWidth: 1400, margin: '24px auto 0' }}><AiRadar radarData={radarData} /></section>
        <NewsSection />
      </div>
